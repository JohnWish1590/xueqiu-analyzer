...
├── cookie_provider.py     # Cookie 统一入口（登录窗/手动/v2/游客）
...
# 登录窗依赖：下载 Playwright 内置 Chromium（只需一次）
playwright install chromium
```

> 本工具**完全自包含**：Cookie 由工具内置的浏览器登录窗或手动粘贴获取，**不需要安装任何 Chrome 插件或「雪哨」等第三方工具**。
...
抓取需要登录态 Cookie。**本工具自带两套取 Cookie 方式，无需安装任何浏览器插件或「雪哨」：**

| 方式 | 说明 |
|------|------|
| 内置登录窗（推荐·默认） | 在「设置 → Cookie 鉴权」点「登录雪球」，工具弹出真实 Chromium 浏览器，你扫码 / 账号密码登录雪球；检测到登录成功后自动读取 `xueqiu.com` 域 Cookie 并本地加密缓存，之后免手动操作。**这是只装了本工具的用户的主路径。** |
| 手动粘贴（兜底） | 展开「或手动粘贴 Cookie」，从浏览器开发者工具（F12 → Network → 任意雪球请求 → 复制 `Cookie` 请求头）粘贴进来，点「保存 Cookie」。适合无图形界面 / 登录窗不便的环境。 |
| 复用 v2 加密库（可选） | 若你同时使用了「雪哨」等 v2 体系工具，可直接复用其加密 Cookie 库；不是必须，不装也完全不影响。 |

> ⚠️ **登录窗弹在哪台电脑？** 登录窗是**运行后端 `server.py` 的那台电脑**弹出的图形窗口。因此 `python server.py` 必须在与你桌面同一会话的终端里前台运行（双击 `.bat` / 命令行启动均可），而不能跑在无图形界面的服务器、容器或后台服务里——否则窗口会弹在别的会话，你这边看不到。
> - 若 server 在本机桌面运行：点「登录雪球」会立刻弹出浏览器，登录成功页面自动刷新。
> - 若 server 跑在远程 / 无 GUI 环境：请改用「手动粘贴 Cookie」兜底（见上）。
> - 若点击后页面提示「Chromium 尚未下载」：在命令行执行一次 `playwright install chromium` 即可。

> Cookie 仅在本地使用，不会上传。失效时（雪球会定期失效）抓取会失败并提示，到设置页重新点「登录雪球」或粘贴最新 Cookie 即可。
...
**Q：抓取提示 Cookie 失效？**
雪球 Cookie 会定期失效。到设置页重新走登录窗或粘贴最新 Cookie 即可。
